...
//  OnTheMap
//
//  Created by Oladele Abimbola on 5/31/22.
//
...
class ParseClient{
    ...
    /* Can't figure out how to overwrite student location at the moment. 
     class func overwriteStudentLocation<Body: Encodable>(requestBody: Body, completionHandler:@escaping(Bool, Error?)->Void){
         //encodable type for this request is the same as the one for the post request. The LocationRequest struct is used as the model object.
         ...
         let encoder = JSONEncoder()
         do{
             let putObject = try encoder.encode(body)
             request.httpBody = putObject
         }catch{
             print(error)
         }
